...
const Dashboard = () => {
  ...
  const loadSensorData = async () => {
    ...
    try {
      ...
      const data = await fetchSensorData(currentPage, dataPerPage, startDate, endDate);
  ...
  const calculateAverages = (data) => {
    // Just return the latest reading from the first item in the data array
    if (data.length === 0) {
      return {
    ...
    return {
      soilMoisture: {
        0: latestReading.soilMoisture[0] !== "Not working" ? latestReading.soilMoisture[0] : null,
        1: latestReading.soilMoisture[1] !== "Not working" ? latestReading.soilMoisture[1] : null
      },
      airTemp: latestReading.dht22?.temp || null,
      airHumidity: latestReading.dht22?.hum || null,
      soilTemp: latestReading.soilTemp?.value || null,
        ...
        <>
          ...
          <div className="speedometers mb-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              {/* Soil Temperature */}
              {!hiddenSensors.includes('soil-temp') && (
                <Speedometer
                  key="soil-temp"
                  id="soil-temp"
                  label="Soil Temperature"
                  value={averageData.soilTemp || "Not working"}
                  max={100}  // Raw ADC value max
                  unit="°C"
                  onHide={handleHideSensor}
                />
          ...
          <div className="charts mb-12">
            <h2 className="text-2xl font-semibold mb-6 text-center">Sensor Data Charts</h2>
